...
function getCardType(cardIndexs, levelPoint) {
    ...
    if (cardIndexs.length === 1) {
        ...
        for (let i = 1; i <= cardIndexs.length - 3; i += 2) {
            ...
            console.log(pokers[cardIndex], pokers[cardIndexs[i - 1]])
...
function isMaster(cardIndexs, levelPoint, master) {
    ...
    if (pokers[cardIndexs[0]].suit === master || pokers[cardIndexs[0]].suit === 'nt'
        || pokers[cardIndexs[0]].point === levelPoint || pokers[cardIndexs[0]].point === '2'
    ) {
...
function compare(cardIndexs1, cardIndexs2, levelPoint, master) {
    ...
    if (isMaster1) {
        ...
        if (isMaster2) {
            ...
            return comparePoint(cardIndexs1[0], cardIndexs2[0], levelPoint, master)
        } else {
        ...
        if (isMaster2) {
            ...
            //比较大小
            return comparePoint(cardIndexs1[0], cardIndexs2[0])
        }
    }
}
 ...
 * @param {*} cardIndex2 
 */
function comparePoint(cardIndex1, cardIndex2) {

    return pokers[cardIndex1].value < pokers[cardIndex2].value
}
...
function sortCards(cardIndexs) {
    ...
    console.log(cardIndexs.map(function(index){
        return pokers[index]
    }))
}